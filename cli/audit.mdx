...
<Note>
The `audit` command supports 6 formats: `console`, `text`, `json`, `html`, `markdown`, `llm`. For additional formats (`xml`, `sarif`), use the `report` command after running an audit:
```bash
squirrel audit https://example.com
squirrel report <audit-id> --format xml
```
