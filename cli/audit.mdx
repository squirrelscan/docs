...
| `--format` | `-f` | Output format: `console`, `text`, `json`, `html` | `console` |
...
<Note>
The `audit` command supports 4 formats: `console`, `text`, `json`, `html`. For additional formats (`xml`, `llm`, `markdown`), use the `report` command after running an audit:
```bash
squirrel audit https://example.com
squirrel report <audit-id> --format llm
```
See the [report command docs](/cli/report) for details on all available formats.
</Note>
