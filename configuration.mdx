...
| `request_method` | `"standard"` \| `"browser_impersonate"` | `"browser_impersonate"` | Request method. `browser_impersonate` uses TLS fingerprint impersonation to bypass WAF detection. |
| `impersonate_browser` | `"chrome"` \| `"firefox"` | `"chrome"` | Browser to impersonate when using `browser_impersonate` method. |
...
If browser impersonation causes issues or you prefer standard fetch:
