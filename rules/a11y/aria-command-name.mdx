...
Command elements (buttons, links, menu items) must have accessible names. Options:

- Add text content
- Use `aria-label` or `aria-labelledby`
- Add a `title` attribute
- For icon-only buttons, use `aria-label` to describe the action (e.g., `aria-label="Close"`)
