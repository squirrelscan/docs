...
Lists (`<ul>` and `<ol>`) should only contain `<li>` elements as direct children. For custom components, you can also use elements with `role='listitem'`.

Move other content inside `<li>` elements.
