...
Never set `aria-hidden="true"` on the `<body>` element. This makes the entire page invisible to assistive technology.

If you need to hide content when a modal is open, add `aria-hidden` to sibling elements of the modal, not to body.
