...
Ensure ARIA attribute values match the expected type:

- Boolean attributes should be `true` or `false`
- Enumerated attributes like `aria-current` have specific allowed values
- Numeric attributes like `aria-level` must be numbers

Check the WAI-ARIA specification for valid values.
