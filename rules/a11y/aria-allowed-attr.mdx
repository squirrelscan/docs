...
Some ARIA attributes are not appropriate for certain roles or elements. For example:

- `role="presentation"` should not have `aria-label` since it removes semantic meaning
- Decorative images (`alt=""`) should not have `aria-label`

Remove conflicting attributes or reconsider the element's role.
