...
<Note>
The `llm`, `xml`, and `markdown` formats are only available via `squirrel report <audit-id> --format <format>`, not directly during audit.
</Note>
...
# Export in LLM-optimized format and pipe to Claude
