...
**Note:** The `llm`, `xml`, and `markdown` formats are only available via the `report` command, not during `audit`. See [Reports documentation](/reports) for details.
...
<Note>
LLM format is only available via the `report` command:
</Note>
...
# Export in LLM format
squirrel report <audit-id> --format llm

# Pipe directly to AI agent
squirrel report <audit-id> --format llm | claude "analyze and prioritize fixes"
```
...
<Note>
XML format is only available via the `report` command:
</Note>
...
squirrel report <audit-id> --format xml | xmllint --format -
```
